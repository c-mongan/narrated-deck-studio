...
export async function startReviewServer(workspaceRoot: string, host = "127.0.0.1", port = 0): Promise<ReviewServerHandle> {
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      const body = gate ? await reviewBody(manifest, gate, token) : `<div class="eyebrow">Narrated Deck Studio</div><h1>Project status</h1><p class="notice">There is no approval waiting right now.</p><pre>${escapeHtml(JSON.stringify(publicProjectStatus(manifest), null, 2))}</pre>`;
