...
export async function requestRevision(manifest: ProjectManifest, target: string, instruction: string, actor = "user", cancelRunning = false): Promise<void> {
  ...
  if (cancelRunning) await writeFile(path.join(manifest.workspaceRoot, "private", "cancel-running"), `${new Date().toISOString()}\n`, { mode: 0o600 });
